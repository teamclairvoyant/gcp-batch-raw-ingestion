  ...
  stages {
    stage('version') {
      steps {
        ...
        sh 'ls  -l /bitnami/jenkins/home/workspace/dataflow-python-etl@tmp/jenkins'
      }
    }
    stage('Running requirements.txt') {
      steps {
      // slackSend color: 'good', message: "Hi <@$userId> Running requirements.txt "
       sh 'pip3 install -r /bitnami/jenkins/home/workspace/dataflow-python-etl@tmp/dataflow/requirements.txt'
      }
    }
      ...
      steps {
        dir('/bitnami/jenkins/home/workspace/dataflow-etl/dataflow/'){
         // slackSend color: 'good', message: "Hi <@$userId> Running setup.py"
          sh 'python3 /bitnami/jenkins/home/workspace/dataflow-python-etl@tmp/dataflow/setup.py install --user'
        }
      }
    }
    stage('build the template and deploy on gcs') {
      steps {
       dir('/bitnami/jenkins/home/workspace/dataflow-python-etl@tmp/dataflow/'){
