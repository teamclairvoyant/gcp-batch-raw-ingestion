  ...
  stages {
    ...
    stage('Running requirements.txt') {
      steps {
       ...
       sh 'pip3 install -r /bitnami/jenkins/home/workspace/gcp-batch-raw-ingestion/dataflow/requirements.txt'
      }
    }
    stage('Running setup.py') {
      steps {
        dir('/bitnami/jenkins/home/workspace/gcp-batch-raw-ingestion/dataflow/'){
          slackSend color: 'good', message: "Hi <@$userId> Running setup.py"
          sh 'python3 /bitnami/jenkins/home/workspace/gcp-batch-raw-ingestion/dataflow/setup.py install --user'
        }
      }
    }
